...
// 160 bytes (hex string): version, prevblockhash, merkleroot, timestamp, bits, nonce
var HEADER_PREFIX     = 'brh-';     // brh-<hash> => <string>
...
BlockDb.prototype._addBlockScript = function(b, header, height) {
  var rawHeader = new Buffer(80)
  rawHeader.writeUInt32LE(header.version, 0);
  rawHeader.write(revHex(header.previousblockhash || ZERO_HASH), 4, 32, 'hex');
  rawHeader.write(revHex(header.merkleroot), 36, 32, 'hex');
  rawHeader.writeUInt32LE(header.time, 68);
  rawHeader.write(revHex(header.bits), 72, 4, 'hex');
  rawHeader.writeUInt32LE(header.nonce, 76);
  ...
  return [
    ...
    {
      type: 'put',
      key: HEADER_PREFIX + b.hash,
      value: rawHeader.toString('hex')
    }
  ];
};
...
BlockDb.prototype.add = function(b, height, cb) {
  ...
  Rpc.getBlock(b.hash, function(err, header) {
    if (err) return cb(err);

    var txs = typeof b.tx[0] === 'string' ? b.tx : b.tx.map( function(o){ return o.txid; });

    var dbScript = self._addBlockScript(b, header, height);
    dbScript = dbScript.concat(self._addTxsScript(txs, b.hash, height));
    self.txDb.addMany(b.tx, function(err) {
      if (err) return cb(err);
...
BlockDb.prototype.getTip = function(cb, allowCache) {
  if (typeof allowCache === 'undefined') { allowCache = true; }

  if (allowCache && this.cachedTip){
...
BlockDb.prototype._getRawHeader = function(hash, cb) {
  if (typeof this.cachedRawHeaders[hash] !== 'undefined') {
    return cb(null, this.cachedRawHeaders[hash]);
  }

  var self = this
  db.get(HEADER_PREFIX + hash, function(err, rawHeader) {
    if (!err) {
      self.cachedRawHeaders[hash] = rawHeader
    }
...
/**
 */
BlockDb.prototype.getHeader = function(hash, cb) {
  var self = this;
  db.get(MAIN_PREFIX + hash, function(err, height) {
    if (!err && height === '-1') {
      err = new Error('BlockHash ' + hash + ' not in chain.');
    }
    ...
    self._getRawHeader(hash, function(err, rawHeader) {
      if (err) {
        return cb(err);
      }

      var buffer = new Buffer(rawHeader, 'hex');
      cb(null, {
...
/**
 */
BlockDb.prototype.getRawHeaders = function(from, to, limit, cb) {
  var self = this
  var headers = ''

  function getOne(hash) {
    self._getRawHeader(hash, function(err, header) {
      if (err) { return cb(err); }
      headers += header;
      ...
      db.get(NEXT_PREFIX + hash, function(err, hash) {
        if (err && !err.notFound) { return cb(err); }

        var isFinished = (err && err.notFound)|| headers.length/160 >= limit
        if (isFinished) { return cb(null, headers); }
